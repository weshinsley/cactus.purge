...
main_title <- function(G) {
  ...
  G$cursor %<>% write_at(16, 20, "S", ifelse(audio_avail, 46, 244))
  G$cursor %<>% write_at(34, 20, "ESC", 46)
  G$cursor %<>% write_at(26, 20, ifelse(G$config$audio, "ON ", "OFF"), 195)

  G %<>% fade_text(30, 22, game1, UNICORN, FADE_IN)

  G$cursor %<>% write_at(28,22, "[1]", 46)
  G$cursor %<>% write_at(40,22, "[2]", 46)
  ...
  set_colour(15)
  if (kp == "escape") {
    G$cursor %<>% clear_pic(G$tv_height)
    G$cursor %<>% pos_at(0, G$tv_height + 2)
    G$main_menu_result <- "EXIT"

  } else if (kp %in% c("1", "2")) {
    G$main_menu_result <- c("CACTZ", "CACTUSKI")[as.integer(kp)]
  ...
  while (TRUE) {
    ...
    if (kp == "i") {
      if (page == "I") {
        ...
        G %<>% show_pic(pkg_file("gfx/cactuski-inst.txt"))
        page <- "I"
      }
      ...
      if (page != "H") {
        G$cursor %<>% clear_pic(19)
        G %<>% fade_text(30, 1, "CACTUSKI HALL OF FAME", UNICORN, FADE_IN)
        G %<>% snazzy_scores(user_file("cactuski-hs.csv.xz"), FADE_IN)
        page <- "H"
      } else {
        G %<>% snazzy_scores(user_file("cactuski-hs.csv.xz"), FADE_OUT)
        G %<>% fade_text(30, 1, "CACTUSKI HALL OF FAME", UNICORN, FADE_OUT)
        G %<>% show_pic(pkg_file("gfx/cactuski.txt"))
        page <- "T"
      }
    }
